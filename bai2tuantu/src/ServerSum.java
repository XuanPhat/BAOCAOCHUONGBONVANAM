...
public class ServerSum extends Thread {
    ...
    {
        s2 = new String[100];
        s2 = s1.split(" "); // tách số ra mỗi một khoảng trắng
        switch(s2[0]){
            case "+":
    ...
    public void Send() // gởi qua client
    {
        try {
            output = new PrintWriter(s.getOutputStream());
            output.println(kq);
            System.out.println("Server đã nhận được kết quả là: "+kq);
            output.flush();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
        ServerSum S = new ServerSum(8080); // tạo socket có só hiệu cổng là 8080
